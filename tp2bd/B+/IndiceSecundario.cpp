...
void InsereChavPagDisponivel(Node *node, char chave[TITLE_SIZE], int apontador){
	int i;
	//Percorre as chaves
...
void InsereChavNohDisponivel(Node *node, char chave[TITLE_SIZE], int apontador){
	int i;
	for(i=0; i < node->tamaho; i++){
		if(strcmp(node->chave[i],chave) > 0){
			for(int j=node->tamaho; j>i; j--){
...
NodeAux* InsereChavPagCheia(Node *node, char chave[TITLE_SIZE], int apontador){
	// Trata a inserção da chave para o caso em que a página de dados já está cheia
	int added=0;
    char pivot[TITLE_SIZE];
...
NodeAux* InsereChavNohCheio(Node *node, char chave[TITLE_SIZE], int apontador){
	int added=0;
    char pivot[TITLE_SIZE];
	Node *newNode = criarNovoNohSecundario();
	NodeAux *newParent = (NodeAux *) malloc(sizeof(NodeAux));
...
NodeAux* insereChaveArvore(Node *node, char chave[TITLE_SIZE], int apontador){
...
int pesquisaChavArqIndSec(fstream *caminhoArquivoDados,fstream *caminhoArquivoIndice, char chave[TITLE_SIZE]){
